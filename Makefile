...
# nvprof -m gst_transactions ./gutil.exe data/triangle.g data/triangle.g >& prof.log
# nvprof -m gld_transactions ./gutil.exe data/triangle.g data/triangle.g >& prof.log
...
library = -lcusparse #-lgcov -coverage

objdir = ./objs/
objfile = $(objdir)Util.o $(objdir)IO.o $(objdir)Match.o $(objdir)Graph.o $(objdir)gutil.o

all: gutil.exe

gutil.exe: $(objfile) main/run.cpp 
	$(NVCC) $(EXEFLAG) -o gutil.exe main/run.cpp $(objfile) $(library)
...
$(objdir)Match.o: match/Match.cu match/Match.h gutil/matrix.cuh gutil/gutil.cuh
	$(NVCC) $(CFLAGS) match/Match.cu -o $(objdir)Match.o  $(library)

$(objdir)gutil.o: gutil/init.cu gutil/init.h
	$(NVCC) $(CFLAGS) gutil/init.cu -o $(objdir)gutil.o  $(library)
...
tarball:
	tar -czvf gutil.tar.gz main util match io graph Makefile README.md objs
